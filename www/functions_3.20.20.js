...
String.prototype.replaceAll = function (find, replace) {
   ...
   //   height: height,
      ...
      function linePlotData(x_values, y_values, y_ses, y_labels,ses_i2, showSEs, ucl_ses,y_LCLUCL) {
      var newdata = [], newSE = [];
      var x = x_values.map(toNumber);

      //var ucl_ses2 = ucl_ses;
      ...
      var ses_i = selectIndexes(y_ses, keep_i);
      //var ses_il = selectIndexes(y_ses, keep_i).replaceAll(",","+");;
      //var ses_i3 = selectIndexes(ses_i2, keep_i); 
      var y_lab = selectIndexes(y_labels, keep_i);
      
       var y_i_line_raw =  y_i.toString().split(',');
      ...
      var y_i_line =  y_i_line_raw.map(Number); 
       var ci_line_num =  ci_test_line.map(Number); 
      var male_num_line = ses_i.slice(0,1).toString().split(',').map(Number);
      var fem_num_line = ses_i.slice(-1).toString().split(',').map(Number);
       
      lcl_num_line2 = ci_line_num.filter(function(value, index, Arr) {
      return index % 2 == 0;      });
      
      ucl_num_line2 = ci_line_num.filter(function(value, index, Arr) {
      return index % 2 == 1;      });
...
const LCLUCL_SEerror = UCL_new.concat(LCL_new);


      for (i = 0; i <= SE_split.length; i = i + 2) {
      if (SE_split[i] !== undefined) {
       lcl.push(SE_split[i]);
        ucl.push(SE_split[i + 1]);
         LCLnum=lcl.map(Number);
           UCLnum=ucl.map(Number).reverse();
         }
      }
      
      var LCLnum_new =[];var UCLnum_new =[];
      Array.prototype.move = function (from, to) {
      this.splice(to, 0, this.splice(from, 1)[0]);
       };
       
       LCLnum_new=LCLnum.move(1,3);
         UCLnum_new=UCLnum.move(1,3);
      
      var ytest_split = y.toString().split(",");  // just split once
        LCL_array.push(SE_split[0]) // before the comma
        UCL_array.push(SE_split[1]); // after the comma
     var ytest=parseFloat(ytest_split);
     var LCLnum1=parseFloat(LCL_array);
     var UCLnum1=parseFloat(UCL_array);
     var LCLdiff=ytest-LCLnum1;
      var UCLdiff=ytest-UCLnum1;
     
    var LCLUCL= [LCLnum1,UCLnum1];
      var LCLUCL_diff= [LCLdiff,UCLdiff]; 
      
     
      var se = ses_i[j].map(toNumber);
      //var lclline2 = lcl_num_line2[j].map(toNumber);
      //var uclline2 = ucl_num_line2[j].map(toNumber);
      ...
      var LCLnum_split= splitArray(LCLnum, null_i);
      var UCLnum_split= splitArray(UCLnum, null_i);
      
      for (var k = 0; k < y_split.length; k++) {
      var y_segment = y_split[k];
      var x_segment = x_split[k];
      var se_segment = se_split[k];
      var LCLnum_segment = LCLnum_split[k];
      var UCLnum_segment = UCLnum_split[k];
      var y_LCLUCL=LCLnum.concat(UCLnum);
      //var y_LCLUCL=LCLnum.concat(UCLnum.reverse());
    //  var LCL_line = y_segment.map(function(num, idx) {return num - lcl_num_line2[idx];});
     // var UCL_line = y_segment.map(function(num, idx) {return num - ucl_num_line2[idx];});
            ...
            var LCL_line3 = y_i_line.map(function(num, idx) {return num - lcl_num_line2[idx];});
      var UCL_line3 = ucl_num_line2.map(function(num, idx) {return num - y_i_line[idx];});

      var LCL_line = y_i_line.map(function(num, idx) {return num - LCL_line3[idx];});
      var UCL_line = y_i_line.map(function(num, idx) {return num + UCL_line3[idx];}); 
...
matrixify(a, 2, 4);
      ...
      var y_se = LCL_line.concat(UCL_line);
      
      var UCLnum2 = UCLnum.reverse();
    ...
    //  },
      ...
      array: UCL_new,
      arrayminus: LCL_new,
      //arrayminus: UCL_line3.map(function(x) {return x;}),
      //array: LCL_line3.map(function(x) {return x;}),
      //arrayminus: UCL_line3.map(function(x) {return x;}),
      },
      text: ses_i[j],
