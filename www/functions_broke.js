...
function barPlotData(x_values, y_values, y_ses, y_labels, lcl_test,ucl_test, lcl_num, ucl_num, showSEs, hideYaxis) {
    ...
    var ses = transpose(ses_j);
     // var ses2 =  ses_j.toString().split(',');
         var ci_test =  ses.toString().split(',');
   var lcl_char = ci_test[0];
   var ucl_char = ci_test[ci_test.length - 1];
  // var lcl_num = ses.slice(0,1); //works but doesn't split -1.96,1.96--treats it as one array
   //var lcl_num =  [lcl_test*1];
    // var ci_test_num = ses.toString().split(',').map(Number);//works but breaks and doesn't treat as array
   var lcl_num = ses.slice(0,1).toString().split(',').map(Number);//works but breaks and doesn't treat as array
   var ucl_num = ses.slice(-1).toString().split(',').map(Number);//works but breaks and doesn't treat as array
     var y_num = y.slice(0).toString().split(',').map(Number);
     // var lcl_num = ses.slice(0,1);//works but breaks and doesn't treat as array
   //var ucl_num = ses.slice(-1);//works but breaks and doesn't treat as array
  // var ci_test_num = ses.toString().split(',').map(Number);//works but breaks and doesn't treat as array
   //var lcl_num = ci_test_num.slice(0,1);//works but breaks and doesn't treat as array
   //var ucl_num = ci_test_num.slice(-1);//works but breaks and doesn't treat as array
    //var ucl_num = [ ucl_test*1];
    
    //var ci_test2 = transpose(ci_test);
    //var ci_test2 = transpose(ci_test_j);
  ...
  //hovertemplate: 'text',
           ...
           // text: y[j]+" ("+ses_j[j]+")",
           //    hoverinfo: "x+text",
         //   text: y_j[j]+" ("+lcl_num[j]+ ","+ ucl_num[j]+")", //works but only keeps 1st row
        // text: y[j]+" ("+lcl_num[j]+ ","+ ucl_num[j]+")",
       // text: y[j]+' ('+ ses[j]+')'+ses2[j],
        //   text: y[j]+' ('+lcl_test[j] +','+ ucl_test+')',
        marker: {
  ...
  //hovertemplate: 'text',
          //  text: y_j[j]+" ("+lcl_num[j]+ ","+ ucl_num[j]+")",  //works but only keeps 1st row
         //       text: y[j]+" ("+lcl_num[j]+ ","+ ucl_num[j]+")",
         //text:  x+"test"+ucl_num[j],
          //text: lcl_test[j]+ucl_test[j]+"test",
            //text: '('+ses[j]+')',
            //text: 'test'
          //text: 'ses[j]'
          //array: ses[j].map(function(x) {return x;})
...
//var newarray = ses[j].map(myFunction)
