...
//        var newstr = str.replaceAll(",","").replace("*","");
   ...
   //   height: height,
     ...
     var lcl_num_line2 = ci_test_line.filter(function(value, index, Arr) {
      return index % 2 == 0;      });
      
     
     var  ucl_num_line2 = ci_test_line.filter(function(value, index, Arr) {
      return index % 2 == 1;      });



//function lcltest(lcl_num_line2) {  
 // lcltest2 = lcl_num_line2.replace(" ","")  ;
 // .replace(" M7-12, Jul-Dec",".5");  
 // .replace(" Q3, Jul-Sep",".5")  
 // .replace(" Q4, Oct-Dec",".75");  
//  lcltest3 = toNumber(lcltest2);  
//  return(lcltest3);  
//}  

//      function trim1 (lcl_num_line2) {
//    return lcl_num_line2.replace(/\s/g,'');
//}

//var lcltest4=lcltest3.map(toNumber);
// function trim1 (lcltest) {return lcltest.replace(/\s+/g,""); }
//function trim12 (lcl_num_line2) {
//    lcl_num_line2 = lcl_num_line2.replace(/^\s+/, '');
//    for (var i = lcl_num_line2.length - 1; i >= 0; i--) {
//        if (/\S/.test(lcl_num_line2.charAt(i))) {
//            lcl_num_line2 = lcl_num_line2.substring(0, i + 1);
//            break;
//        }
//    }
//    return lcl_num_line2;
//}


     //  lcl_test =lcl_num_line2.replace(" ", "");

     // var  lcl_num_line222 = ucl_num_line2.replace(" ", ""); 
      
     lcl_num_line22=lcl_num_line2.map(toNumber);
   
      ucl_num_line22=ucl_num_line2.map(toNumber);   
      
      //lcl_num_line2 = lcl_num_line.filter(function(value, index, Arr) {
     // return index % 2 == 0;      });
      
      //use lcl_num_line //
      //ucl_num_line2 = lcl_num_line.filter(function(value, index, Arr) {
      //return index % 2 == 1;      });
...
//var a = [8.17,6.5,8.69,9.48];
var a =lcl_num_line22;
...
var bb = matrixify(b, 2, 2);
      ...
      var lcl_num_line22_split = splitArray(lcl_num_line22, null_i);
      var ucl_num_line22_split = splitArray(ucl_num_line22, null_i);
      
      for (var k = 0; k < y_split.length; k++) {
      var y_segment = y_split[k];
      var x_segment = x_split[k];
      var se_segment = se_split[k];
      var lcl_segment =lcl_num_line22_split[k];
      var ucl_segment = ucl_num_line22_split[k];
      
      var LCL_line = y_segment.map(function(num, idx) {return num - lcl_num_line2[idx];});
      var UCL_line = ucl_num_line2.map(function(num, idx) {return num - y_segment[idx];});
      //var UCL_line = y_segment.map(function(num, idx) {return num - ucl_num_line2[idx];});
      
      var LCL_line2 = y_segment.map(function(num, idx) {return num - lcl_segment[idx];});
      var UCL_line2 = y_segment.map(function(num, idx) {return num + ucl_segment[idx];});
     // var UCL_line2 = y_segment.map(function(num, idx) {return num - UCL_line[idx];});
      ...
      var y_se = aaa.concat(bbbb).reverse();
     // var y_se = aaa.concat(bbb.reverse());
      //var y_se = LCL_line2.concat(UCL_line2.reverse());
