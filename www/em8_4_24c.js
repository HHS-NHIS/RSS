...
$(document).ready(function () {
    ...
    */
    ...
    //from https://stackoverflow.com/questions/35987969/select-only-one-option-from-2-dropdowns
    // $(document).on("change", ".round2", function (e) {
    //     $('.round1').val('default');
    // });

    // $(document).on("change", ".round1", function (e) {
    //     $('.round2').val('default');
    // });

    $(function () {
        // document.getElementById("round2").value = "gyno_ov_remove";
        ...
        $(document).ready(function () {
            $("#round2").change(function () {
                ...
                stat_var3 = $("#round2 option:selected").val();
                console.log($val)
            });
        });

        $(document).ready(function () {
           // $("div.round2 select").val("adhd_two");
            if ($("#year").val() == '2023, Aug-Round1') {
                $("div.round1 select").val("air_cleaner_mold");
                //filename1 = 'air_cleaner_mold' + '__' + rowX + '__' + colX + '__' + subGrp;
                //console.log(filename1);
                $('#round2').val("-1");
            } else if ($("#year").val() == 'January-February, 2024') {
                $("div.round2 select").val("Employment1");
                $('#round1').val("-1");
    ...
    //https://stackoverflow.com/questions/42203800/set-dropdown-value-based-on-what-is-selected-in-another-dropdown-using-javascrip

    var stat_var33;
    var stat_var3;
    $(document).ready(function () {
        $('#round2').change(function () {
            //           //stat_var333 = $("#select2 option:selected").text("test");
            stat_var33 = $("#round2 option:selected").text();
            stat_var3 = $("#round2 option:selected").val();
            //     console.log(stat_var33)
            console.log(stat_var33)
    ...
    $(document).ready(function () {
        $("#year").change(function () {
            if ($(this).val() == '2023, Aug-Round1') {
               // $("#round1").val('air_cleaner_mold');
                //$('select.stat_var2') = $('#round1.stat_var2') 
               // stat_var33 =  $("#round1").txt('To remove dust, pollen, mold, or other allergens');
                $("#round2").val('-1');
                //stat_var33 = $("#round1 option:selected").text();
                //stat_var3 = $("#round1 option:selected").val();
                //console.log(stat_var3)
            } else if ($(this).val() == 'January-February, 2024') {
               // $("#round2").val('adhd_two');
    ...
    //    $("select.stat_var2").change(function () {
    //        //    $("select.stat_var2").change(function () {
    //        var stat_var2 = $(this).find("option:selected").val();
    //        //           // $("p").html("You have selected the country - " + stat_var2);
    //        console.log(stat_var2)
    //   });

    //var stat_var2 = stat_var3;
    ...
    $('#data-view').change(function () {
        ...
        if (isTrend) {
            ...
            $('.year-main label').text('');
        } else {
            $('.hide-if-trend').slideUp('slow');
            $('.year-start').animate({ width: '0%' }, 400);
            $('.year-main label').text('Topic Area:');
        }
    });
    ...
    $('#stat_var, .stat_var2, #col_var, #row_var, #year, #year-start, #data-view, .subgrp').on('change', function () {
    ...
    $(document).on('updateValues', function () {
        ...
        //statName = stat_var33;
        if ($("#year").val() == '2023, Aug-Round1') {
            //statName = $("#round1 option:selected").text();
            statName = $("#round1 option:selected").text().replace(/Select outcome/g, "");
            colgrpName = $("#round1 option:selected").text();
            //$("#round2 option:selected").text("");
            stat_var3 = $("#round1 option:selected").val();
...
//        }
        ...
        filename1 = stat_var3 + '__' + rowX + '__' + colX + '__' + subGrp;
        //filename1 = $('select.stat_var2').val() + '__' + rowX + '__' + colX + '__' + subGrp;
        console.log(filename1)
        $.getJSON(filename1 + '.json', function (data) {
            ...
            var grpName = rowX == 'ind' ? 'Round 3' : $('#row_var option:selected').text();
            table.columns(2).header().toJQuery().text(grpName);


            // For 'use' app, convert header to statistic label if no colgrp selected
            //var colgrpName = $('select.stat_var2').find("option:selected").text();
            //var colgrpName = $('select.stat_var2').find("option:selected").text();
    ...
    $(document).on('updateNotes', function () {
        ...
        newSource = "<b>Source:</b> " + NCHS + ", " + RSS + ", Round 3, January-February, 2024";
      ...
      //      ". Generated interactively: " + today3 + " from https://wwwn.cdc.gov/NHISDataQueryTool///SHS_adult/index.html";
        var newCitation = NCHS + ". " + titleCaption + ". " + RSS +
            ". Generated interactively: " + today3 + " from https://wwwndev.cdc.gov/NHISDataQueryTool/RSS3/index.html";
    ...
    $(document).ready(function () {
        $("select").change(function () {
            $("select option:selected").each(function () {
                ...
                if ($(this).attr("value") == "2023, Aug-Round1") {
                    $(".box").hide();
                    $("#Round1").show();
                    stat_var33 = $("#round1 option:selected").text();
                    stat_var3 = $("#round1 option:selected").val();
                }
                if ($(this).attr("value") == "January-February, 2024") {
                    $(".box").hide();
                    $("#Round2").show();
                    stat_var33 = $("#round2 option:selected").text();
                    stat_var3 = $("#round2 option:selected").val();
    ...
    const topics = {
        family: [
            { group: 'Beliefs, knowledge, and use of family health history', value: 'Familyhealthhistory13', text: 'Believe knowing family health history is somewhat or very important' },
            { group: 'Beliefs, knowledge, and use of family health history', value: 'Familyhealthhistory19', text: 'Know at least some of mother\'s health history' },
            { group: 'Beliefs, knowledge, and use of family health history', value: 'Familyhealthhistory17', text: 'Know at least some of father\'s health history' },
            { group: 'Beliefs, knowledge, and use of family health history', value: 'Familyhealthhistory18', text: 'Know at least some of maternal grandparents\' health history' },
            { group: 'Beliefs, knowledge, and use of family health history', value: 'Familyhealthhistory20', text: 'Know at least some of paternal grandparents\' health history' },
            { group: 'Beliefs, knowledge, and use of family health history', value: 'Familyhealthhistory14', text: 'Ever actively collected health information for family health history' },
            { group: 'Beliefs, knowledge, and use of family health history', value: 'Familyhealthhistory16', text: 'Ever shared family health history with other relatives' },
            { group: 'Beliefs, knowledge, and use of family health history', value: 'Familyhealthhistory15', text: 'Ever shared family health history with doctor' },

            { group: 'Difficulties collecting health history information', value: 'Familyhealthhistory25', text: 'Think it\'s at least somewhat difficult to collect health history information' },
            { group: 'Difficulties collecting health history information', value: 'Familyhealthhistory21', text: 'Not knowing what to collect makes collecting health history difficult' },
            { group: 'Difficulties collecting health history information', value: 'Familyhealthhistory22', text: 'Organization or storage makes collecting health history difficult' },
            { group: 'Difficulties collecting health history information', value: 'Familyhealthhistory23', text: 'No contact with or deceased relatives makes collecting health history difficult' },
            { group: 'Difficulties collecting health history information', value: 'Familyhealthhistory26', text: 'Discomfort asking relatives makes collecting health history difficult' },
            { group: 'Difficulties collecting health history information', value: 'Familyhealthhistory24', text: 'Some other reason makes collecting health history difficult' },
            // Add more family health history topics here
        ],
        genetic: [
            { group: 'Genetic tests for risk of select conditions', value: 'Genetictesting31', text: 'Ever had genetic test for risk of cancer or heart disease' },
            { group: 'Genetic tests for risk of select conditions', value: 'Genetictesting33', text: 'Interested in genetic test for risk of cancer or heart disease' },
            { group: 'Reasons why had a genetic test', value: 'Genetictesting41', text: 'Want to share genetic test results with their relatives' },
            { group: 'Reasons why had a genetic test', value: 'Genetictesting39', text: 'Knowing genetic testing results would affect their healthcare decisions' },
            { group: 'Reasons why had a genetic test', value: 'Genetictesting40', text: 'Knowing genetic testing results would encourage a change in health habits' },
            { group: 'Reasons why had a genetic test', value: 'Genetictesting38', text: 'A doctor recommended genetic testing' },
            // Add more genetic test topics here
        ],
        sexual: [
            { group: 'Receipt of sexual health services', value: 'Sexualhealth65', text: 'Received any sexual health services, past 12 months' },
            { group: 'Receipt of sexual health services', value: 'Sexualhealth64', text: 'Received sexual health services in doctor\'s office' },
            { group: 'Receipt of sexual health services', value: 'Sexualhealth60', text: 'Received sexual health services in a community or public health clinic' },
            { group: 'Receipt of sexual health services', value: 'Sexualhealth61', text: 'Received sexual health services in a hospital/emergency room' },
            { group: 'Receipt of sexual health services', value: 'Sexualhealth63', text: 'Received sexual health services in an urgent care or in-store health clinic' },
            { group: 'Receipt of sexual health services', value: 'Sexualhealth62', text: 'Received sexual health services in a telehealth appointment' },

            { group: 'Where turn to for information', value: 'Sexualhealth72', text: 'Turned to their family for information on sexual health' }, ,
            { group: 'Where turn to for information', value: 'Sexualhealth73', text: 'Turned to their partner for information on sexual health' },
            { group: 'Where turn to for information', value: 'Sexualhealth74', text: 'Turned to their peers or friends for information on sexual health' },
            { group: 'Where turn to for information', value: 'Sexualhealth75', text: 'Turned to their therapist for information on sexual health' },
            { group: 'Where turn to for information', value: 'Sexualhealth71', text: 'Turned to their doctor for information on sexual health' },
            { group: 'Where turn to for information', value: 'Sexualhealth70', text: 'Turned to the internet or social media for information on sexual health' },
            { group: 'Where turn to for information', value: 'Sexualhealth69', text: 'Turned to another source for information on sexual health' },
            // Add more sexual health topics here
        ]
    };
        ...
        const topicFilter = document.getElementById('round2');
        topicFilter.innerHTML = ''; // Clear previous options

            // Create a map to hold optgroups
            const optgroupMap = {};

            topics[selectedGroup].forEach(topic => {
                if (!optgroupMap[topic.group]) {
                    const optgroup = document.createElement('optgroup');
                    optgroup.label = topic.group;
                    optgroup.style.fontWeight = 'bold';
                    optgroupMap[topic.group] = optgroup;
                    topicFilter.appendChild(optgroup);
                }
                const option = document.createElement('option');
                option.value = topic.value;
                option.textContent = topic.text;
                optgroupMap[topic.group].appendChild(option);
            });

            // Set default selection based on group
            if (selectedGroup === 'genetic') {
                topicFilter.value = 'Genetictesting31'; // Default for Genetic Test group
            } else if (selectedGroup === 'sexual') {
                topicFilter.value = 'Sexualhealth65'; // Default for Sexual Health group
            }
    ...
    document.getElementById('group-filter').dispatchEvent(new Event('change'));



    // Trigger --------------------------------------------------------------------
    // trigger on load too -- must be at end;
    $('#stat_var, #stat_var2, #code-language, #data-view, #col_var, #row_var, #year, #showSEs').trigger('change');
});
