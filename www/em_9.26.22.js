...
//
 ...
 // Check if year strings are quarterly:  
     ...
     // is_quarterly = year.includes("J", "M", "q");  
    ...
    // Export table to csv --------------------------
      $('#dl-table').on('click', function() {
       ...
       // var SE_test  =  array_column_conditional(seCols, 0, 0);
        ...
        var seCaption = "Confidence Intervals for " +
           //   fullCaption.charAt(0).toLowerCase() + fullCaption.slice(1);
              fullCaptionCsv.charAt(0).toLowerCase() + fullCaptionCsv.slice(1);
      ...
      //  var dlSource = newSource.replace('<b>','').replace('</b>','');
          var dlSource2 = newCitationCsv;
        
        var csv = $.grep(
            ['"' + fullCaptionCsv.replace(" (standard errors)","")+'"', coefCSV,
           '"'+seCaption.replace(" (standard errors)","") +'"', seCSV,
           $('#suppress').text(),
           $('#RSE').text(),
                '"' + dlSource2 + '"'], Boolean).join("\n");
 ...
 // Array of Index SE selection -----------------------------------------------------------
    ...
    var layout = {
      ...
      margin: {l: 100, r: 50, b: 110, t: 100},
        legend: { y: 0.2, tracegroupgap: 5, traceorder: 'grouped' }
      ...
      if(isTrend) {
       ...
       // hovertemplate= "hovertemplate";
        ...
        //layout.yaxis = {tickformat: '0,.5',   title: 'Percent'};
          layout.yaxis = { tickformat: '0,.5', hoverformat: hoverfmt, range: [0, y_max2], title: 'Percent' };
        //  layout.yaxis = { tickformat: '0,.5', hoverformat: hoverfmt, title: 'Percentage of Adults' };
...
//          '2018Q1', '2018Q2', '2018Q3', '2018Q4'],  title: 'Year'} : {};
        ...
        layout.xaxis = {showline: false, zeroline: false, hoverformat: hoverfmt, tickformat: '0,.5', title: 'Percentage of Adults'};
    ...
    // Caption, Citation, Notes ---------------------------------------------------
      var fullCaption, fullCaptionCsv, fullCaption3, plotCaption, newSource, statName2, newCitation, tableCaptionCsv;
    ...
    $(document).on('updateNotes', function() {
      ...
      $(document).on('updateNotes', function () {
          ...
          var yearName2 = isTrend ? [year, yearStart].filter(unique).sort().join("-") : year;

          fullCaption = newCaption + ", " + yearName;
          fullCaptionCsv = newCaption + ", " + yearName2;
          ...
          tableCaptionCsv = showSEs ? fullCaptionCsv :
              fullCaptionCsv.replace(" (95% confidence intervals)", "");
          $('#table-captionCsv').text(tableCaptionCsv);
        ...
        //      ". Generated interactively: " + today3 + " from https://wwwn.cdc.gov/NHISDataQueryTool/ER_Biannual/index_biannual.html";
          ...
          newCitation = NCHS + ". " + tableCaption + ". " + NHIS +
              ". Generated interactively: " + today3 + " from https://wwwn.cdc.gov/NHISDataQueryTool/ER_Biannual/index_biannual.html";

          newCitationCsv = NCHS + ". " + tableCaptionCsv + ". " + NHIS +
              ". Generated interactively: " + today3 + " from https://wwwn.cdc.gov/NHISDataQueryTool/ER_Biannual/index_biannual.html";
