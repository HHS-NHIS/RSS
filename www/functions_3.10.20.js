...
function qtr_to_num(str) {  
  str2 = str.replace(" M1-6, Jan-Jun",".0")  
  .replace(" M7-12, Jul-Dec",".5");  
...
function num_to_qtr(num) {  
 ...
 str2 = str.replace(".0"," M1-6, Jan-Jun")  
  .replace(".5"," M7-12, Jul-Dec") ; 
...
//        var newstr = str.replaceAll(",","").replace("*","");
   ...
   //   height: height,
    ...
    //var ci_test_line2 =   ci_test_line.map(Number);
      ...
      var ucl_num_line = ses_i.slice(-1).toString().split(',').map(Number);
       var ci_test_line2 = lcl_num_line.concat(ucl_num_line); 
...
//}
//var a =ci_test_line2;
//var aa = matrixify(a, 2, 2);
//var b =lcl_num_line;
//var bb = matrixify(b, 2, 2);
//var b = [10.35, 8.25,10.99, 11.64];
//var b=lcl_num_line22;
//var bb = matrixify(b, 2, 2);
      
      lcl_num_line2 = lcl_num_line.filter(function(value, index, Arr) {
      return index % 2 == 0;      });
      lcl_num_line22 = lcl_num_line.filter(function(value, index, Arr) {
      return index % 2 == 1;      });
      
      ucl_num_line2 = ucl_num_line.filter(function(value, index, Arr) {
      return index % 2 == 0;      });
      ucl_num_line22 = ucl_num_line.filter(function(value, index, Arr) {
      return index % 2 == 1;      });
      
      lcl_num_line3 = ci_test_line2.filter(function(value, index, Arr) {
      return index % 2 == 0;      });
      
      ucl_num_line3 = ci_test_line2.filter(function(value, index, Arr) {
      return index % 2 == 1;      });
      
      lcl_num_line4 = lcl_num_line3.filter(function(value, index, Arr) {
      return index % 2 == 0;      });
      
      ucl_num_line4 = ucl_num_line3.filter(function(value, index, Arr) {
      return index % 2 == 1;      });
      ...
      var lcl_num_line3_split = splitArray(lcl_num_line3, null_i);
      var ucl_num_line3_split = splitArray(ucl_num_line3, null_i);
      
      for (var k = 0; k < y_split.length; k++) {
      var y_segment = y_split[k];
      var x_segment = x_split[k];
      var lcl_num_line3_segment = lcl_num_line3_split[k];
      var ucl_num_line3_segment = ucl_num_line3_split[k];
      var ci_test2 = ci_test_line2[k];
      
      var LCL_line3 = y_segment.map(function(num, idx) {return num - lcl_num_line3[idx];});
      var UCL_line3old = ucl_num_line3.map(function(num, idx) {return num - y_segment[idx];});
      
      const UCL_line3 = UCL_line3old.filter(function (value) {
      return !Number.isNaN(value);
       });
       
      var leftSide_LCL = LCL_line3.splice(0, Math.floor(LCL_line3.length / 2));
      var leftSide_UCL = UCL_line3.splice(0, Math.floor(UCL_line3.length / 2));
      var LCL_line = y_segment.map(function(num, idx) {return num - lcl_num_line3[idx];});
      var UCL_line_old = ucl_num_line3.map(function(num, idx) {return num - y_segment[idx];}); 
      
      
          const UCL_line = UCL_line_old.filter(function (value) {
      return !Number.isNaN(value);
       });
      ...
      var y_se_old = LCL_line.concat(UCL_line.reverse());
      
      
      const y_se = y_se_old.filter(function (value) {
      return !Number.isNaN(value);
       });
    ...
    //  },
      ...
      array: ucl_num_line3_segment.map(function(x) {return x;}),
      arrayminus: lcl_num_line3_segment.map(function(x) {return x;}),
      },
      text: ses_i[j],
